...
if __name__ == '__main__':
    ...
    # 多个关键字模式
        ...
        f = open(f'{error_dir_name}' + r'/' + 'error_keyword.txt', 'r+', encoding='utf-8')
        last_kw = f.read().strip('\n')
        try:
            print('-' * 50 + f'正在爬取：{last_kw}' + '-' * 50)
            # 下载图片函数
            download_pic(last_kw, nums)
            print('-' * 50 + f'爬取完成' + '-' * 50)
            print()
        except:
            print(f'error：关键字【{last_kw}】爬取失败 ！')
            # 记录报错、报错的keyword
            error_log = open(f'{error_dir_name}' + r'/' + 'error_log.txt', 'a', encoding='utf-8')
            error_log.write(f'error：关键字【{last_kw}】爬取失败 ！\n')
            error_keyword = open(f'{error_dir_name}' + r'/' + 'error_keyword.txt', 'a', encoding='utf-8')
            error_keyword.write(f'{last_kw}\n')
        f.close()
